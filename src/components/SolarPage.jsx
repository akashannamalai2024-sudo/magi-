...
export default function SolarPage({ onNavigate }) {
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF', overflow: 'hidden' }}>
      ...
      {/* SECTION 8 — PROJECT CONSULTANT (INTERACTIVE WIZARD) */}
      <section id="consultant" className="section section-secondary">
        <div className="container" style={{ maxWidth: '720px' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <span style={{ color: 'var(--primary)', fontSize: '14px', fontWeight: '700', letterSpacing: '1.5px', textTransform: 'uppercase' }}>
              SOLAR ESTIMATOR
            </span>
            <h2 style={{ fontSize: '36px', color: 'var(--text-dark)', marginTop: '12px', marginBottom: '12px' }}>
              Find the Right Solar Solution
            </h2>
            <p style={{ fontSize: '16px', color: 'var(--text-muted)' }}>
              Configure your basic parameters for a custom engineered proposal.
            </p>
          </div>
          ...
          <div style={{
                ...
                {[1, 2, 3, 4, 5].map(stepNum => (
                  <div 
                    ...
                    style={{
                      ...
                      flex: 1,
                      borderRadius: '3px',
                      backgroundColor: stepNum <= wizardStep ? 'var(--primary)' : 'var(--border-color)',
            ...
            {!wizardCompleted ? (
              <form onSubmit={handleWizardSubmit}>
                {wizardStep === 1 && (
                  <div>
                    <h3 style={{ fontSize: '20px', color: 'var(--text-dark)', marginBottom: '24px', fontWeight: '700' }}>
                      1. Select Your Organization Type
                    </h3>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
                      {['Industrial Manufacturing', 'Educational Institution', 'Commercial/Retail Office', 'Government Infrastructure', 'Residential Community'].map(option => (
                        <button
                          key={option}
                          type="button"
                          onClick={() => {
                            setWizardData(prev => ({ ...prev, orgType: option }));
                            handleNextStep();
                          }}
                          style={{
                            padding: '16px 20px',
                            textAlign: 'left',
                            borderRadius: '12px',
                            border: wizardData.orgType === option ? '2px solid var(--primary)' : '1px solid var(--border-color)',
                            backgroundColor: wizardData.orgType === option ? 'rgba(31, 93, 55, 0.04)' : '#FFFFFF',
                            color: 'var(--text-dark)',
                            fontWeight: '600',
                            fontSize: '15px',
                            cursor: 'pointer',
                ...
                {wizardStep === 2 && (
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
                      {['Below ₹25,000', '₹25,000 – ₹1,00,000', '₹1,00,000 – ₹5,00,000', 'Above ₹5,00,000'].map(option => (
                        <button
                          key={option}
                          type="button"
                          onClick={() => {
                            setWizardData(prev => ({ ...prev, billRange: option }));
                            handleNextStep();
                          }}
                          style={{
                            padding: '16px 20px',
                            textAlign: 'left',
                            borderRadius: '12px',
                            border: wizardData.billRange === option ? '2px solid var(--primary)' : '1px solid var(--border-color)',
                            backgroundColor: wizardData.billRange === option ? 'rgba(31, 93, 55, 0.04)' : '#FFFFFF',
                            color: 'var(--text-dark)',
                            fontWeight: '600',
                            fontSize: '15px',
                            cursor: 'pointer',
                    ...
                    <button type="button" onClick={handlePrevStep} style={{ marginTop: '24px', background: 'none', border: 'none', color: 'var(--primary)', fontWeight: '700', cursor: 'pointer' }}>
                      ← Back
                    </button>
                  </div>
                )}
                ...
                {wizardStep === 3 && (
                  <div>
                    <h3 style={{ fontSize: '20px', color: 'var(--text-dark)', marginBottom: '24px', fontWeight: '700' }}>
                      3. Available Spatial Footprint
                    </h3>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
                      {['Small Rooftop (Under 5,000 sq ft)', 'Medium Rooftop (5,000 – 20,000 sq ft)', 'Large Corporate Roof (Above 20,000 sq ft)', 'Ground Level Land Area'].map(option => (
                        <button
                          key={option}
                          type="button"
                          onClick={() => {
                            setWizardData(prev => ({ ...prev, roofArea: option }));
                            handleNextStep();
                          }}
                          style={{
                            padding: '16px 20px',
                            textAlign: 'left',
                            borderRadius: '12px',
                            border: wizardData.roofArea === option ? '2px solid var(--primary)' : '1px solid var(--border-color)',
                            backgroundColor: wizardData.roofArea === option ? 'rgba(31, 93, 55, 0.04)' : '#FFFFFF',
                            color: 'var(--text-dark)',
                            fontWeight: '600',
                            fontSize: '15px',
                            cursor: 'pointer',
                ...
                {wizardStep === 4 && (
                  <div>
                    <h3 style={{ fontSize: '20px', color: 'var(--text-dark)', marginBottom: '24px', fontWeight: '700' }}>
                      4. Preferred System Configuration
                    </h3>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px' }}>
                      {['Grid-Tied Solar (No battery backup)', 'Hybrid Solar (Integrated battery storage)', 'Off-Grid (Autonomous system)', 'I Need Engineering Advice'].map(option => (
                        <button
                          key={option}
                          type="button"
                          onClick={() => {
                            setWizardData(prev => ({ ...prev, systemType: option }));
                            handleNextStep();
                          }}
                          style={{
                            padding: '16px 20px',
                            textAlign: 'left',
                            borderRadius: '12px',
                            border: wizardData.systemType === option ? '2px solid var(--primary)' : '1px solid var(--border-color)',
                            backgroundColor: wizardData.systemType === option ? 'rgba(31, 93, 55, 0.04)' : '#FFFFFF',
                            color: 'var(--text-dark)',
                            fontWeight: '600',
                            fontSize: '15px',
                            cursor: 'pointer',
                ...
                {wizardStep === 5 && (
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                        <input 
                          ...
                          style={{ padding: '14px', borderRadius: '8px', border: '1px solid var(--border-color)', outline: 'none' }}
                        />
                        <input 
                          type="text" 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '32px' }}>
                      <button type="button" onClick={handlePrevStep} style={{ background: 'none', border: 'none', color: 'var(--primary)', fontWeight: '700', cursor: 'pointer' }}>
                        ← Back
                      </button>
                      <button 
                        type="submit" 
              ...
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <h3 style={{ fontSize: '24px', color: 'var(--text-dark)', marginBottom: '12px', fontWeight: '700' }}>
                  Configuration Received
                </h3>
                <p style={{ fontSize: '15px', color: 'var(--text-muted)', lineHeight: '1.6', marginBottom: '32px', maxWidth: '480px', marginInline: 'auto' }}>
                  Thank you, <strong>{wizardData.name}</strong>. An engineering consultant will review your average bill ({wizardData.billRange}) and site space ({wizardData.roofArea}) to compile a customized solar structural draft for <strong>{wizardData.company}</strong>.
                </p>
                <button 
                  type="button" 
                  onClick={() => {
                    ...
                    setWizardData({ orgType: '', billRange: '', roofArea: '', systemType: '', name: '', company: '', phone: '', email: '', city: '' });
                  }}
